...
// === v1.2.3 EXE compatibility endpoints ===
app.get('/api/tasks/generate', async function(req, res) {
  try {
    ...
    if (!whales.length) return res.json({ success: true, count: 0, message: 'No whales found' });
    ...
    for (var i = 0; i < whales.length; i++) {
      ...
      taskStore[hostId].push({
        id: 't_' + Date.now() + '_' + count,
        taskId: 't_' + Date.now() + '_' + count,
        host_id: hostId, profileId: w.username, videoUrl: w.profile_url,
        commentText: 'Hai ' + (w.nickname || w.username) + '! Mampir ke live kita ya! \u{1F525}',
